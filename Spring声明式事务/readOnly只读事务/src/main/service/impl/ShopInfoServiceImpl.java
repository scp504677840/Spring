...
public class ShopInfoServiceImpl implements ShopInfoService {
     ...
     * @param bookName  书籍名称
     * @Transactional public void batchBuy(){
     ...
     * 事务的回滚
     * 对哪些异常进行回滚事务
     * rollbackFor = QueryException.class
     * 对哪些异常不进行回滚事务
     * noRollbackFor = UpdateException.class
     * 注意：
     * 1.以上两个属性都不建议自己设置，建议使用默认值。
     * 2.两个属性值都可以指定Class类型和Class全类名。
     * ---------------------------------------------------------------
     * 只读事务
     * 什么是只读事务？
     * 若事务被设置为只读事务，那么这个事务开启后，只能读取数据，不能更新数据。
     * 如何设置只读事务？
     * 将@Transactional的readOnly属性赋值为true即可，默认为false。
     * 应用场景：
     * 建议将查询数据的方法事务属性一定要设置为只读事务。
     * 例如：@Transactional(readOnly = true)
     */
    @Transactional(propagation = Propagation.REQUIRED,
            isolation = Isolation.READ_COMMITTED,
            rollbackFor = QueryException.class, noRollbackFor = UpdateException.class,
            readOnly = true)
    @Override
